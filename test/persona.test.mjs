...
test("standing persona uses engagement-only windows and deliberate response placement", () => {
  ...
  assert.match(prompt, /Capacity changes are an owner action, not an agent tool/);
  assert.match(prompt, /The only Services MCP capability mounted here is the read-only managed-session capacity reader/);
  ...
  assert.match(prompt, /post a concise blocker before starting/);
  ...
  assert.doesNotMatch(prompt, /setUsageWindow/);
