...
test("standing persona uses engagement-only windows and deliberate response placement", () => {
  ...
  const firstPrice = prompt.indexOf("read its public live catalog and price");
  ...
  assert.match(prompt, /Do not use an alder-services MCP server: it is not mounted/);
  assert.match(prompt, /create a payment grant with intent: establish/);
  assert.match(prompt, /Services POST \/connections/);
  ...
  assert.match(prompt, /createMerchantConnectionRecoveryProof for the selected existing pma_/);
